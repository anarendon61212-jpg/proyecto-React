...
const SignIn: React.FC = () => {
  ...
  const [microsoftUserInfo, setMicrosoftUserInfo] = useState<any>(null);

  const googleButtonRef = useRef<HTMLDivElement>(null);

  const handleGoogleContainerClick = () => {
    // Buscar el botón de Google dentro del contenedor y hacer click programáticamente
  ...
  const handleRoleSelect = async (role: 'TEACHER' | 'STUDENT') => {
    ...
    // Determinar si es Google o Microsoft

    const isGoogle = googleUserInfo !== null;

    const userInfo = isGoogle ? googleUserInfo : microsoftUserInfo;

    

    try {

      console.log('Iniciando registro:', { isGoogle, userInfo, role });
      ...
      if (isGoogle) {
        ...
        console.log('Registrando con Google:', { googleUserInfo, role });

        // Registrar usuario con Google usando el endpoint existente del backend

        const result = await googleAuthService.registerWithGoogle(googleUserInfo, role);
        ...
        console.log('Registrando con Microsoft:', { microsoftUserInfo, role });

        // Registrar usuario con Microsoft usando el mismo enfoque que Google

        const result = await microsoftAuthService.registerWithMicrosoft(microsoftUserInfo, role);
      ...
      toast.success(`Usuario registrado como ${role === 'TEACHER' ? 'Docente' : 'Estudiante'} con código ${user.code}`, { id: 'register' });
  ...
  const handleMicrosoftLogin = async () => {
    ...
    try {
      ...
      const [given_name, ...lastNameParts] = displayName.split(" ");

      const userInfo = {

        email: firebaseUser.email || "",

        given_name: given_name || firebaseUser.email || "",
      ...
      console.log("Microsoft userInfo:", userInfo);



      // Verificar si el usuario ya existe en el backend (como hace Google)
      ...
      try {
        ...
        if (data.data && data.data.length > 0) {
          ...
          // El usuario ya existe, permitir login
          ...
          // Simular login exitoso con Microsoft para usuarios existentes
      ...
      toast.error('Error al autenticar con Microsoft');
  ...
  return (
    ...
    <>
      ...
      {/* Modal para selección de rol - Google y Microsoft */}
      ...
      <RoleSelectionModal
        ...
        userName={googleUserInfo?.name || microsoftUserInfo?.name || ''}
          ...
          <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
            ...
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              ...
              <div className="flex w-full items-center justify-center mt-4">
                ...
                <div
                  ref={googleButtonRef}
                  onClick={handleGoogleContainerClick}
                  className="flex w-full items-center justify-center gap-3.5 rounded-lg border border-stroke bg-gray p-4 hover:bg-opacity-50 dark:border-strokedark dark:bg-meta-4 dark:hover:bg-opacity-50 cursor-pointer"
                >
