...
const EvaluationRubricAssociation = () => {
  ...
  const visibleEvaluations = useMemo(() => {
    ...
    return evaluations
      .filter((evaluation) =>
        onlyWithoutRubric ? !evaluation.rubric_id : true,
      )
      .filter((evaluation) => {
        if (!normalizedSearch) {
          return true;
  ...
  const loadData = async () => {
    ...
    try {
      const [evaluationsResponse, rubricsResponse, subjectsResponse, groupsResponse] =
        await Promise.all([
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-sm border border-stroke bg-white px-5 pb-6 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
          ...
          {loading ? (
            ...
            <div className="rounded border border-dashed border-stroke px-5 py-8 text-center dark:border-strokedark">
              <p className="mb-3 text-bodydark2">
                No hay rubricas publicadas disponibles para asociar.
              </p>
              <Link
                to="/evaluation/rubrics"
                className="inline-flex rounded bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-opacity-90"
              >
                Ir a publicar rubricas
              </Link>
            </div>
